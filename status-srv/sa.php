<?
 /*
  *  status-srv/oc.php: South america pool
 ...
 $pool = $status_collection->get_pool("SA"); 
 ...
 foreach($top_servers as $server=>$weight)
 {
    ...
    // End correct for own host
  	...
  	if($serverarr[$server]->get_ipv6() !== false && $include_ipv6) 
		$ip6=gethostbyname6($server);
  	else
		$ip6="";
  

   	echo  "_pgpkey-http._tcp.sa.pool SRV 0 $weight 11371 ".$server.".\n";
   	echo  "sa.pool A ".$ip."\n";

   	if($serverarr[$server]->get_ipv6() !== false && $include_ipv6 && $ip6 != "") 
   	{
		echo "sa.pool AAAA ".$ip6."\n";
   	}
 }
?>
