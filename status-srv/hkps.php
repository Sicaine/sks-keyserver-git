<?
 ...
 foreach($serverarr as $server)
 {
 	...
 	if(!$server->get_has_hkps())
		continue;

	if($server->get_affected_cve2014_3207())
		continue; 


    $array_of_all_online_servers[] = array($server->get_hostname(), $server->get_ipv6(), $server->get_hkps_port());
 }
 ...
 foreach($array_of_selected_servers as $server)
 {
	  ...
	  if($server[2] == 443 && $server[1] && $include_ipv6)
	  {
	  		$ip=gethostbyname6($server[0]);
			if(!is_array($ip) && $ip != "")
	  			echo "hkps.pool AAAA ".$ip."\n";
	  }
	  
	  // Disabled due to GnuPG bug 1446 and 1447
	  //echo "_pgpkey-https._tcp.hkps.pool SRV 100 100 {$server[2]} {$server[0]}.\n";
 }
?>
